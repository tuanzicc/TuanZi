...
namespace TuanZi.Reflection
{
	public class FastActivator
	{
		...
		{
            Check.NotNull(constructorInfo, nameof(constructorInfo));
		...
		private static void CheckIsValidType(Type type)
		{
			...
			{
				throw new ArgumentException($"The type to create activators for must be concrete. Type: {type.ToString()}", nameof(type));
			}
		}
